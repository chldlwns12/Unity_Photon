...
//MonoBehaviourPunCallbacks : 포톤서버 접속, 로비 접속, 룸 접속 등
public class NetworkManager : MonoBehaviourPunCallbacks
{
    public Text textInfo;           //네트워크 상태를 보여줄 택스트
    ...
    public void OnConnect()
    {
        ...
        {
            //룸(게임세상)으로 바로 접속 실행
            textInfo.text = "랜덤방에 접속...";
            PhotonNetwork.JoinRandomRoom();
        }
        else
        {
            //마스터 서버에 접속중이 아니라면 다시 마스터 서버에 접속하기
            //접속 정보 표시하기
            textInfo.text = "오프라인 : 마스터 서버와 연결실패 \n 접속 재시도 중";
            //마스터 서버에 접속하는 함수 (이놈이 젤 중요)
            PhotonNetwork.ConnectUsingSettings();
    ...
    {
        //접속 정보 표시하기
        textInfo.text = "빈 방이 없으니 새로운 방 생성중...";
        //빈방을 생성한다
